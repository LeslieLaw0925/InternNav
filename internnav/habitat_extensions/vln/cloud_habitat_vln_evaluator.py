...
# ------------------------------------- model ------------------------------------------
    ...
    def cloud_agent_step(self, rgb, depth=None, text=None):
    ...
    def _run_eval_dual_system(self) -> tuple:  # noqa: C901
        ...
        while self.env.is_running:
            ...
            while (not done) and (step_id <= self.max_steps_per_episode):
                ...
                if stage == 's2':
                    # S2 inference
                    response_data = self.cloud_agent_step(look_down_image, episode_instruction)
                    action = response_data['action']
                    stage = response_data['stage']

                    if stage == 's2' and action == action_code.STOP:
