...
@Evaluator.register('cloud_habitat_vln')
class CloudHabitatVLNEvaluator(DistributedEvaluator):
    def __init__(self, cfg: EvalCfg):
        ...
        self.cloud_base_url = f"http://192.168.105.15:8023"
        # self.cloud_base_url = f"http://192.168.105.5:30814"
...
# ------------------------------------- model ------------------------------------------
    ...
    def _init_agents(self, agent_cfg):
        self.cloud_agent_name = self._init_cloud_agent(agent_cfg)
    
    def _init_cloud_agent(self, agent_cfg):
        agent_cfg.model_name = 'habitat_cloud_agent'
        request_data = InitRequest(agent_config=agent_cfg)
        response = requests.post(
            url=f'{self.cloud_base_url}/agent/init',
    ...
    def cloud_agent_step(self, rgb, depth=None, text=None, step_id=None, look_down=False):
        obs = {'rgb': rgb, 'depth': depth, 'instruction': text, 
               'step_id': step_id, 'look_down': look_down}
        ...
        response = requests.post(
            url=f'{self.cloud_base_url}/agent/{self.cloud_agent_name}/step',
            json=request_data,
            headers={'Content-Type': 'application/json'},
        )
        response.raise_for_status()
        ...
        total_cloud_time = response_data.pop('processing_time')
        transmission_delay = cloud_latency - total_cloud_time
        self.inference_logger.record_by_key(system_perf.TRANSMISSION, transmission_delay)

        s1_time = response_data.pop('s1_time', None)
        s2_time = response_data.pop('s2_time', None)
    ...
    def cloud_agent_reset(self, reset_index=None):
        response = requests.post(
            url=f'{self.cloud_base_url}/agent/{self.cloud_agent_name}/reset',
        ...
        while self.env.is_running:
            ...
            while (not done) and (step_id <= self.max_steps_per_episode):
                ...
                if stage == 's2':
                    ...
                    response_data = self.cloud_agent_step(look_down_image, depth=None, 
                                                          text=episode_instruction, 
                                                          step_id=step_id, 
                                                          look_down=look_down)
                    action = response_data['action']
                    stage = response_data['stage']

                    if stage == 's1' and action == action_code.STOP:
