...
@Evaluator.register('dist_habitat_vln')
class DistributedHabitatVLNEvaluator(DistributedEvaluator):
    def __init__(self, cfg: EvalCfg):
        ...
        self.s1_base_url = f"http://192.168.105.10:8023"
...
# ------------------------------------- model ------------------------------------------
    ...
    def _init_agents(self, agent_cfg):
        self.s1_agent_name = self._init_s1_agent(agent_cfg)
        self.s2_agent_name = self._init_s2_agent(agent_cfg)

    def _init_s1_agent(self, agent_cfg):
        agent_cfg.model_name = 'habitat_s1_agent'
        request_data = InitRequest(agent_config=agent_cfg)
        response = requests.post(
            url=f'{self.s1_base_url}/agent/init',
    ...
    def s2_agent_step(self, rgb, text=None, step_id=None, look_down=None):
        ...
        if self.set_adaptive_compression:
            ...
            elasped_time = time.time() - self.step_start_time
            if_compressed = (estimated_cloud_time is not None) and \
                (estimated_cloud_time > (self.cloud_latency_threshold - elasped_time))
            if if_compressed:
    ...
    def s1_agent_step(self, rgb, depth, traj_latents=None):
        ...
        response = requests.post(
            url=f'{self.s1_base_url}/agent/{self.s1_agent_name}/step',
            json=request_data,
            headers={'Content-Type': 'application/json'},
        )
        response.raise_for_status()
            ...
            while (not done) and (step_id <= self.max_steps_per_episode):
                ...
                if len(action_seq) == 0 and pixel_goal is None:
                    ...
                    if traj_latents is not None:
                        ...
                        s1_response = self.s1_agent_step(look_down_image, look_down_depth, traj_latents)
                        ...
                        if action == action_code.STOP:
                            ...
                            self.inference_logger.record_by_key(system_perf.STEP, time.time() - self.step_start_time)
                ...
                if len(action_seq) != 0:
                    ...
                    if len(local_actions) == 0:
                        ...
                        s1_response = self.s1_agent_step(look_down_image, look_down_depth)
